...
class _UserDetailsState extends State<UserDetails> {
  Future<List<QueryDocumentSnapshot<Map<String, dynamic>>>> _getUserDatas(
    ...
    try {
      QuerySnapshot<
          Map<String,
              dynamic>> profileQuerySnapshot = await FirebaseFirestore.instance
          .collection('users')
          .doc(uid) // Access the user document with UID
          .collection(
              'profiles') // Access the 'profiles' collection under the user document
          .doc(uid) // Access a specific profile document (if needed)
          .collection(
              'chatbot_responses') // Access the 'chatbot_responses' subcollection
          .get();
